...
def BalasHammer(datalist,order,provision):
    ...
    for i in datalist:
        ...
        data.append(temp)
    cpt=0
    # As long as there are still cells that can be filled, loops
    while(notallzero(ordercp)==0 and notallzero(provisioncp)==0):
        ...
        min1=None
        min2=None
        # Finds the 2 minimal costs of every line
        for i in datalistcp:
            for j in i:
                if min1==None:
                    min1=j
            ...
            for i in range(len(datalistcp)):
                if min1 is None:
                    ...
                    if datalistcp[i][j] < min1:
                        ...
                        min2 = datalistcp[i][j]
            # Computes penalty for every column
            if min2==100000000000 and min1!=100000000000:
                columnPenalty.append(min1)
            else:
                columnPenalty.append(min2 - min1)
        # Finds maximum penalty among lines 
        maxi=-1
        ...
        maxi=-1
        # Finds maximum penalty among column
        for i in range(len(columnPenalty)):
            if columnPenalty[i]>maxi:
                maxi=columnPenalty[i]
                changej=i
        
        # Chooses randomly maximal penalty if some of them are equal
        if maxi==0:
            changej=randint(0,len(ordercp)-1)
        # Checks that the chosen cell hasn't been filled yet then adds the maximal quantity possible in the cell
        if data[changei][changej]==None:
            ...
            # Changes order and provision tempporarily to reflect the change of quantity
