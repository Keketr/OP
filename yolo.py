...
while True:
    ...
    start_time = time.perf_counter()  # Start measuring time for the algorithm
    if i == "1":
    ...
    elapsed_time = time.perf_counter() - start_time  # End measuring time
    print(f"Execution time for {'NorthWest' if i == '1' else 'BalasHammer'}: {elapsed_time:.4f} seconds")
    ...
    # Suppose you want to continue with further processing like "marche-pied"
    # You would start another timing here if needed and implement that phase
    a = 0
    z = None
    while z != "1" and z != "2":
